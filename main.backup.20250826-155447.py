...
def _load_restrooms_from_csv(path: str) -> int:
    """restrooms.csv를 읽어 RESTROOMS_DB에 로드"""
    RESTROOMS_DB.clear()
    if not os.path.exists(path):
        return 0

    required = [
        "운영노선명","역명","지상 또는 지하 구분","역층",
        "게이트 내외 구분","근접 출입구 번호","상세위치","전화번호","개방시간"
    ]

    with open(path, "r", encoding="utf-8-sig", newline="") as f:
        reader = csv.DictReader(f)
        if not reader.fieldnames or any(col not in reader.fieldnames for col in required):
            missing = [c for c in required if not reader.fieldnames or c not in reader.fieldnames]
            raise HTTPException(status_code=400, detail=f"CSV 헤더가 맞지 않습니다. 누락: {', '.join(missing)}")

        for row in reader:
            rec = {
                "line_name": (row.get("운영노선명") or "").strip(),     # 예: '2호선'
                "station_name": (row.get("역명") or "").strip(),        # 예: '홍대입구'
                "ground": (row.get("지상 또는 지하 구분") or "").strip(), # '지상'/'지하'
                "level": (row.get("역층") or "").strip(),               # B2/B1/1F...
                "gate_area": (row.get("게이트 내외 구분") or "").strip(),# '내부'/'외부'
                "near_exit_no": (
                    int((row.get("근접 출입구 번호") or "").strip())
                    if (row.get("근접 출입구 번호") or "").strip().isdigit() else None
                ),
                "detail_location": (row.get("상세위치") or "").strip(),
                "tel": (row.get("전화번호") or "").strip(),
                "hours": (row.get("개방시간") or "").strip(),
            }
            RESTROOMS_DB.append(rec)
    return len(RESTROOMS_DB)

# (선택) 서버 시작 시 자동 로드 시도
try:
    os.makedirs(os.path.dirname(RESTROOMS_PATH), exist_ok=True)
    _ = _load_restrooms_from_csv(RESTROOMS_PATH)
except Exception:
    pass
...
# =========================
def _save_rest_db():
    os.makedirs(os.path.dirname(REST_DB_PATH), exist_ok=True)
    with open(REST_DB_PATH, "w", encoding="utf-8") as f:
        json.dump(REST_DB, f, ensure_ascii=False)

def _load_rest_db():
    if os.path.exists(REST_DB_PATH):
        try:
            with open(REST_DB_PATH, "r", encoding="utf-8") as f:
                REST_DB.update(json.load(f))
        except Exception:
            pass  # 손상/빈 파일이어도 진행
...
async def restrooms_upload(file: UploadFile = File(...)):
    ...
    with open(RESTROOMS_PATH, "wb") as out:
        out.write(contents)
    rows = _load_restrooms_from_csv(RESTROOMS_PATH)
    return {"saved_as": RESTROOMS_PATH, "rows_loaded": rows}

@app.get("/restrooms/count")
def restrooms_count():
    return {"rows": len(RESTROOMS_DB)}
...
def toilets_search(
    ...
    metas = [m for m in REST_DB.values() if _station_eq(m.get("station_name"), station)]
    if line_no:
...
async def ask_llm(body: Dict[str, Any] = Body(...)):
    ...
    try:
        ...
        if intent.get("want_restroom"):
            ...
            cand_names.update({m.get("station_name") for m in REST_DB.values() if m.get("station_name")})
            if not st_name:
            ...
            pool = [m for m in REST_DB.values() if _station_eq(m.get("station_name"), st_name)]
            if line_no:
